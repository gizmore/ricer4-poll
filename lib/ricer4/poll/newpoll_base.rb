module Ricer4::Plugins::Poll
  class NewpollBase < Ricer4::Plugin
    ...
    def announce(question)
      ...
      get_plugin('Poll/Poll').announce_targets do |target|
        usercount += target.is_a?(Ricer4::User) ? 1 : 0
        channelcount += target.is_a?(Ricer4::Channel) ? 1 : 0
    ...
    def close_trigger(question)
      "$T$#{get_plugin('Poll/Close').plugin_trigger} #{question.id}"
    end
    def answer_trigger(question)
      "$T$#{get_plugin('Poll/Vote').plugin_trigger} #{question.id}"
    end
    ...
    def poll_announce_message(question)
      t(:msg_announce,
        user: question.creator.display_name,
